...
class SPT3GPrototype(InstallableLikelihood):
    ...
    def initialize(self):
        ...
        path_to_emu = '/disks/shear16/canasherrera/NUTS_meets_pocoMC/jaxcapse/trained_emu/'
        self.Capse_TT = jaxcapse.load_emulator(path_to_emu + "TT/")
        self.Capse_TE = jaxcapse.load_emulator(path_to_emu + "TE/")
        self.Capse_EE = jaxcapse.load_emulator(path_to_emu + "EE/")
    ...
    def logp(self, **data_params):
        ...
        #print(self.provider.get_param('omegam'))
        #print(self.provider.get_param('H0'))
        #Cls = self.provider.get_Cl(ell_factor=True)
        #print(Cls)
        input_test = np.array([self.provider.get_param('logA'), 
                               self.provider.get_param('ns'), 
                               self.provider.get_param('H0'), 
                               self.provider.get_param('ombh2'), 
                               self.provider.get_param('omch2'), 
                               self.provider.get_param('tau')])
        Cl_TT_capse = self.Capse_TT.get_Cl(input_test)
        Cl_TE_capse = self.Capse_TE.get_Cl(input_test)
        Cl_EE_capse = self.Capse_EE.get_Cl(input_test)
        Cls_capse = {'ell': self.ells,
                     'tt': np.insert(Cl_TT_capse[:3249], 0, 0),
                     'te': np.insert(Cl_TE_capse[:3249], 0, 0),
                     'ee': np.insert(Cl_EE_capse[:3249], 0, 0),
                     }
        return self.loglike(Cls_capse.get("te"), Cls_capse.get("ee"), **data_params)


class TEEEemu(SPT3GPrototype):
    r"""
